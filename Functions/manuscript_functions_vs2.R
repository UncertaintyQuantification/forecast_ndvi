...
lm_mc_propagation_ndvi <- function(testing_index = 11:18, 
                                   output_ndvi, 
                                   input_vpd, 
                                   input_precip, 
                                   n_mc = 100) {
  
  num_loc <- nrow(output_ndvi)
  num_test <- length(testing_index)
  
  # Initialize storage matrices
  pred_median_record <- matrix(NA, num_loc, num_test)
  interval_length_record <- matrix(NA, num_loc, num_test)
  prop95_record <- matrix(NA, num_loc, num_test)
  
  for(i_test in testing_index) {
    
    test_idx <- i_test - min(testing_index) + 1
    print(paste("Processing Test Year Index:", i_test))
    
    for(i_loc in 1:num_loc) {
      
      
      y_ndvi <- as.vector(output_ndvi[i_loc, 1:(i_test-1)])
      x_vpd <- as.vector(input_vpd[i_loc, 1:(i_test-1)])
      x_precip <- as.vector(input_precip[i_loc, 1:(i_test-1)])
      time_index <- 1:(i_test-1)
      
      # VPD Model
      mod_vpd <- lm(x_vpd ~ time_index)
      pred_vpd <- predict(mod_vpd, newdata = data.frame(time_index = i_test), interval = 'predict')
      # Recover predictive SD using the t-distribution quantile (since lm uses t-dist for intervals)
      df_vpd <- mod_vpd$df.residual
      sd_vpd <- (pred_vpd[,"upr"] - pred_vpd[,"fit"]) / qt(0.975, df_vpd)
      
      # Precip Model
      mod_precip <- lm(x_precip ~ time_index)
      pred_precip <- predict(mod_precip, newdata = data.frame(time_index = i_test), interval = 'predict')
      df_precip <- mod_precip$df.residual
      sd_precip <- (pred_precip[,"upr"] - pred_precip[,"fit"]) / qt(0.975, df_precip)
      
      # Sample Covariates (n_mc times)
      vpd_mc_samples <- rnorm(n_mc, mean = pred_vpd[,"fit"], sd = sd_vpd)
      precip_mc_samples <- rnorm(n_mc, mean = pred_precip[,"fit"], sd = sd_precip)
      
      
      # Train NDVI model on historical data
      df_train_ndvi <- data.frame(August_NDVI = y_ndvi, 
                                  avg_vpd_Jul_Aug = x_vpd, 
                                  avg_precip_Jan_Aug = x_precip)
      mod_ndvi <- lm(August_NDVI ~ avg_vpd_Jul_Aug + avg_precip_Jan_Aug, data = df_train_ndvi)
      
      # Predict NDVI for all n_mc covariate samples simultaneously
      df_test_ndvi <- data.frame(avg_vpd_Jul_Aug = vpd_mc_samples, 
                                 avg_precip_Jan_Aug = precip_mc_samples)
      pred_ndvi_mc <- predict(mod_ndvi, newdata = df_test_ndvi, interval = 'predict')
      
      # Recover the predictive SD for the NDVI predictions
      df_ndvi <- mod_ndvi$df.residual
      sd_ndvi_mc <- (pred_ndvi_mc[,"upr"] - pred_ndvi_mc[,"fit"]) / qt(0.975, df_ndvi)
      
      # Pre-allocate a matrix to hold the samples 
      # (Rows = n_mc samples per pair, Cols = n_mc covariate pairs)
      ndvi_sample_matrix <- matrix(NA, nrow = n_mc, ncol = n_mc)
      
      # Loop through each covariate sample's mean and sd
      for(j in 1:n_mc) {
        current_mean <- pred_ndvi_mc[j, "fit"]
        current_sd <- sd_ndvi_mc[j]
        
        # Sample n_mc times for THIS specific mean and sd pair
        ndvi_sample_matrix[, j] <- rnorm(n_mc, mean = current_mean, sd = current_sd)
      }
      
      # Flatten the matrix into one giant vector
      ndvi_final_samples <- as.vector(ndvi_sample_matrix)
      
      
      true_ndvi <- output_ndvi[i_loc, i_test]
      
      # Calculate quantiles using the massive flattened vector
      median_val <- median(ndvi_final_samples)
      lower_95 <- quantile(ndvi_final_samples, 0.025)
      upper_95 <- quantile(ndvi_final_samples, 0.975)
      
      # Save to matrices
      pred_median_record[i_loc, test_idx] <- median_val
      interval_length_record[i_loc, test_idx] <- upper_95 - lower_95
      prop95_record[i_loc, test_idx] <- (true_ndvi > lower_95) & (true_ndvi < upper_95)
  ...
  # Return final list
  return.list <- list(
    pred_lm_median = pred_median_record,
    interval_length_lm_model = interval_length_record,
    prop95_lm_model = prop95_record
  )
