...
vpd=np.loadtxt(prefix+'vpd_matrix.txt')
precip=np.loadtxt(prefix+'precip_matrix.txt')

ndvi=np.loadtxt(prefix+'ndvi_matrix.txt')
...
for i in range(14000):
    ...
    train_data = grid_features[:, 0:9, :]
    test_data = grid_features[:, 9:17, :]
    train_targets = grid_ndvi[:, 1:10, :]
...
rmse=(sum(l2_losses)/14000)**0.5
rmse

predictions_np = all_predictions.cpu().numpy()
np.savetxt(prefix + "all_predictions_RNN.txt", predictions_np, fmt="%.6f")
